...
if(!class_exists('BoixEquipe'));
    require_once _PS_MODULE_DIR_.'boix_mdf/classes/BoixEquipe.php';

use Cleandev\BoixMdf\Classes\Equipe;

class AdminBoixEquipeController extends ModuleAdminController {

    public function __construct()
    {
        $this->table = 'boix_equipe';
        $this->className = 'BoixEquipe';
        $this->lang = true;
        $this->bootstrap = true;

        $this->deleted = false;
        $this->allow_export = true;
        $this->list_id = 'boix_equipe';
        $this->identifier = 'id_boix_equipe';
        $this->_defaultOrderBy = 'id_boix_equipe';
        ...
        $this->fields_list = array(
            ...
            'name'=>array(
                'title'=>$this->l('Nom', [], 'Modules.Boixmdf.Adminboixequipecontroller.php'),
                'width'=>'auto'
            ),
            'role'=>array(
                'title'=>$this->l('Role', [], 'Modules.Boixmdf.Adminboixequipecontroller.php'),
                'width'=>'auto'
            ),
            'active' => array(
    ...
    {
        if (!($equipe = $this->loadObject(true))) {
            return;
        }

        $image = Equipe::getImgPath(false).DIRECTORY_SEPARATOR.$equipe->id.'.jpg';
        $image_url = ImageManager::thumbnail(
            $image,
            $this->table.'_'.(int)$equipe->id.'.'.$this->imageType,
            350,
            $this->imageType,
            true,
            true
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'textarea',
                    'label' => $this->l('Role', [], 'Modules.Boixmdf.Adminboixequipecontroller.php'),
                    'name' => 'role',
                    'lang' => true,
                    'cols' => 60,
                    'rows' => 10,
        ...
        foreach ($this->_languages as $language) {
            $this->fields_value['role_'.$language['id_lang']] = htmlentities(Tools::stripslashes($this->getFieldValue(
                $equipe,
                'role',
                $language['id_lang']
            )), ENT_COMPAT, 'UTF-8');
